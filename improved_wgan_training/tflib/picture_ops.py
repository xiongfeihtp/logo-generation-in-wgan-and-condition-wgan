...
def process_image(encoded_image, config, is_training, thread_id=0):
...
def build_inputs(config, is_training):
    ...
    for thread_id in range(config.num_preprocess_threads):
        ...
        # image = process_image(encoded_image, thread_id=thread_id)
        images_and_label.append([encoded_image, image_label, image_name])
