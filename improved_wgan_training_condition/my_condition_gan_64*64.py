...
@file: my_gan_64*64.py
@time: 2018/4/26 下午4:09
...
"""
tensorflow中cpu和gpu中支持的图片格式有差别，cpu只支持一个，gpu两个都支持
"""
...
def conv_cond_concat(value, cond):
    ...
    # 构造对应的conditional
    value_shapes[1] = cond_shapes[-1]
    cond = tf.expand_dims(cond, axis=2)
    cond = tf.expand_dims(cond, axis=3)
    cond_map = cond * tf.ones(value_shapes)
    return tf.concat([value, cond_map], 1)
...
def Normalize(name, axes, inputs):
    if ('Discriminator' in name) and (MODE == 'wgan-gp'):
        ...
        result = lib.ops.batchnorm.Batchnorm(name, axes, inputs, fused=True)
        return result
...
def ResidualBlock(name, labels, input_dim, output_dim, filter_size, inputs, resample=None, he_init=True):
    """
    resample: None, 'down', or 'up'
    """
    """
    conditional gan中维度对齐的部分
    """
    if resample == 'down':
    ...
    output = inputs
    # conditional
    # batchsize channel+conditional height width
    output = conv_cond_concat(output, labels)
    output = Normalize(name + '.BN1', [0, 2, 3], output)
    output = tf.nn.relu(output)
    input_dim = output.get_shape().as_list()[1]
    if resample == 'down':
...
# ! Generators
def GoodGenerator(n_samples, labels, noise=None, dim=DIM, nonlinearity=tf.nn.relu):
    if noise is None:
        noise = tf.random_normal([n_samples, 128])
    # conditional
    noise = tf.concat([noise, labels], axis=1)
    input_dim = noise.get_shape().as_list()[-1]
    output = lib.ops.linear.Linear('Generator.Input', input_dim, 4 * 4 * 8 * dim, noise)
    # batchsize channel height width
    output = tf.reshape(output, [-1, 8 * dim, 4, 4])
    # conditional
    output = ResidualBlock('Generator.Res1', labels, 8 * dim, 8 * dim, 3, output, resample='up')
    output = ResidualBlock('Generator.Res2', labels, 8 * dim, 4 * dim, 3, output, resample='up')
    output = ResidualBlock('Generator.Res3', labels, 4 * dim, 2 * dim, 3, output, resample='up')
    output = ResidualBlock('Generator.Res4', labels, 2 * dim, 1 * dim, 3, output, resample='up')
...
# ! Discriminators
def GoodDiscriminator(inputs, labels, dim=DIM):
    output = tf.reshape(inputs, [-1, 3, 64, 64])
    output = lib.ops.conv2d.Conv2D('Discriminator.Input', 3, dim, 3, output, he_init=False)
    # conditional
    output = ResidualBlock('Discriminator.Res1', labels, dim, 2 * dim, 3, output, resample='down')
    output = ResidualBlock('Discriminator.Res2', labels, 2 * dim, 4 * dim, 3, output, resample='down')
    output = ResidualBlock('Discriminator.Res3', labels, 4 * dim, 8 * dim, 3, output, resample='down')
    output = ResidualBlock('Discriminator.Res4', labels, 8 * dim, 8 * dim, 3, output, resample='down')
...
with g.as_default():
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
        is_train = tf.get_variable(
            "is_train", shape=[], dtype=tf.bool, trainable=False)

        global_step = tf.get_variable('global_step', shape=[], dtype=tf.int32,
                                      initializer=tf.constant_initializer(0), trainable=False)

        images, labels, _ = build_inputs(config, is_train)
        labels = tf.one_hot(labels, config.CLASS, 1, 0)
        ...
        fake_data = Generator(BATCH_SIZE, labels)

        disc_real = Discriminator(real_data, labels)
        disc_fake = Discriminator(fake_data, labels)
        ...
        all_fixed_noise_samples = Generator(BATCH_SIZE, labels, noise=fixed_noise)
        ...
        for _ in range(ITERS):
            ...
            # if iteration % config.period == 0:
            #     loss_sum = tf.Summary(value=[tf.Summary.Value(
            #         tag="model/loss", simple_value=_disc_cost), ])
